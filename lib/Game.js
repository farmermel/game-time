...
class Game {
  ...
  generatePlanets(ctx) {
    var planetSizes = {};
    var planetSrcImg = {};
    this.getPlanetSource(planetSrcImg);
    this.checkPlanetSize(planetSizes);
    if (planetsArray.length < 10) {
      var planet = new Planet(Math.floor(Math.random()* 460), - Math.floor(Math.random()* 600), planetSrcImg.arrayVal, planetSizes.width, planetSizes.height);
  ...
  getPlanetSource(planetSrcImg) {
    let planetSrc = ['../images/mars.svg', '../images/jupiter.svg', '../images/moon.svg'];
    let planetIndex = Math.floor(Math.random() * 3);
    planetSrcImg.arrayVal = planetSrc[planetIndex];
  }

  checkPlanetSize(planetSizes) {
    if(this.level === 0) {
